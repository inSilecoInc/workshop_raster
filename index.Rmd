...
### Find cell values with [`raster`](https://CRAN.R-project.org/package=raster)
...
---
# Basic cell-based computations  

### Find cell values with [`stars`](https://CRAN.R-project.org/package=stars)
...
---
# Change map projection [`raster`](https://CRAN.R-project.org/package=raster)
...
rar_m <- projectRaster(rar, crs = rar_crs)
projection(rar_m)
# plot(rar_m)
```
...
---
# Change map projection [`stars`](https://CRAN.R-project.org/package=stars)

```{R projrar2, eval = FALSE}
ras_m <- st_transform(ras, crs = 3857)
st_crs(ras_m)
```
...
ras_2 <- st_warp(ras, cellsize = 0.01, crs = st_crs(ras)) 
