...
rar_template1 <- raster(mr, xmn = -71, xmx = -55, ymn = 43, ymx = 55, crs = projection(rar))
plot(rar_template1)
```
...
```{R rar_warp3, eval = FALSE}
rar_template2 <- raster(xmn = -71, xmx = -55, ymn = 43, ymx = 55, crs = projection(rar), resolution = .25)
...
ras_template1 <- st_as_stars(st_bbox(ras), nx = 21, ny = 21, values = runif(21
   *21))
...
```{R, ras_stc}
ras_stc <- c(ras, ras*2, along = "bath_v2")  
# ras_stc <- c(ras, ras*2)  
# ras_stc <- stack(list(bath_v1 = rar, bath_v2 = rar2))             
plot(ras_stc)
plot(st_apply(ras_stc, c(1,2), sum))    
```
